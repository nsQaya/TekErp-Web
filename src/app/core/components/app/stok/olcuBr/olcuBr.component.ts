...
export class OlcuBrComponent implements AfterViewInit, OnInit {
	...
	constructor(private olcuBrService:OlcuBrService, private lookupService:LookUpService,private alertifyService:AlertifyService,private formBuilder: FormBuilder, private authService:AuthService) { }
	...
	addOlcuBr(){
		...
		this.olcuBrService.addOlcuBr(this.olcuBr).subscribe(data => {
			this.getOlcuBrList();
	...
	updateOlcuBr(){
		...
		this.olcuBrService.updateOlcuBr(this.olcuBr).subscribe(data => {
...
simge : ["", Validators.required]
	...
	deleteOlcuBr(olcuBrId:number){
		this.olcuBrService.deleteOlcuBr(olcuBrId).subscribe(data=>{
	...
	getOlcuBrById(olcuBrId:number){
		this.clearFormGroup(this.olcuBrAddForm);
		this.olcuBrService.getOlcuBrById(olcuBrId).subscribe(data=>{
			this.olcuBr=data;
