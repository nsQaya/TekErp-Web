...
import { IBaseResponseValue, IStokKod } from "../../../utils/types";
...
export default (props: IStokKodModalProps) => {
  ...
  const onSubmit= useCallback(async()=>{
    ...
    let response : AxiosResponse<IBaseResponseValue<IStokKod>, any>;

    if(id==0){
      response= await api.stokKod2.create({
        adi: name,
        kodu: code,
        aktarimDurumu: 0,
      });
    }else{
      response= await api.stokKod2.update({
        id: id,
        adi: name,
        kodu: code,
        aktarimDurumu: 0
  ...
  return (
    <Modal show={isShowing} onHide={() => props.onHide && props.onHide()}>
      <Modal.Header closeButton>
        <Modal.Title>Stok Kod 2 Ekle</Modal.Title>
      </Modal.Header>
      <Modal.Body>
